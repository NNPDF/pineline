...
with open(f"new_results/{dataset}.json") as user_file:
    new = np.array(json.load(user_file))

with open(f"master_results/{dataset}.json") as user_file:
    master = np.array(json.load(user_file))
...
ratio = (np.float64(fonll_result) / master_result).reshape(len(uniqx), len(uniqq2)).T
ratio = np.flip(ratio, axis=0)
pd_ratio = pd.DataFrame(ratio, columns=np.around(uniqx, 6), index=np.around(np.flip(uniqq2), 1))
