...
In order to include NNLO contributions for a given observable we will need to produce grids at the desired order for each obserable.
...
it is necessary to choose an appropiate parton-level fixed-order generator.
For this exercise we have choosen [`vrap`](https://www.slac.stanford.edu/~lance/Vrap/)[^3].
