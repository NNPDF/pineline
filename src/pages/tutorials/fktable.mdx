...
```
where the name of the dataset, that will be used from now on, is `HERACOMBNCEP460`. Notice that
the name of the `yaml` file needs to be the same of the dataset (so in this case `HERACOMBNCEP460.yaml`).
...
our example it would be `HERACOMBNCEP460`). An example of the output of the command
```bash
...
a single grid, the command would iterate over all the grids, producing an operator card 
...
fine, you should see something similar to this line at the end of the log file
```bash
