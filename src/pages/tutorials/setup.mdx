...
- The `template` operator card that provides some of the inputs needed by _EKO_
  and some placeholders to be filled by _PINEKO_.
...
automatically writes the relevant `operator card` for the requested dataset
using the provided `template` and `grid`. An example of such runcard for the
dataset `HERACOMBNCEP460` is
